...
const DestinationInputApi = ({ onPlanTrip }) => {
  ...
  useEffect(() => {
    const loadDestinations = async () => {
      try {
        const destinations = await api.getDestinations(true);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!destination.trim() || !dateRange.from || !dateRange.to) {
      toast({
        ...
        description: "Please fill in all fields before planning your trip.",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      // Test if we can get travel plans for this destination
      await api.getTravelPlans(destination.trim());
      
      // If successful, proceed with trip planning
      onPlanTrip({
      ...
      toast({
        ...
        description: `Loading travel plans for ${destination.trim()}...`
      });
      
    } catch (error) {
      toast({
        title: "Destination Not Available",
        description: `Sorry, we don't have travel plans for "${destination}" yet. Try one of our popular destinations.`,
        variant: "destructive"
      });
    } finally {
      setLoading(false);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-50 to-orange-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-amber-800 text-lg">Loading travel destinations...</p>
        </div>
      </div>
    );
  }
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="flex flex-wrap gap-2 mt-4">
                ...
                {popularDestinations.map((place) => (
                  <button
                    ...
                    disabled={loading}
                    className="px-3 py-1 text-sm bg-amber-100 text-amber-800 rounded-full hover:bg-amber-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {place.name}
                  </button>
                ))}
            ...
            <div className="pt-6">
              <Button
                ...
                disabled={!destination.trim() || !dateRange.from || !dateRange.to || loading}
              >
                {loading ? (
                  <>
...
export default DestinationInputApi;